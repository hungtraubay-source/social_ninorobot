...
class RealRobotLyapunovFollower(Node):
    def __init__(self):
        ...
        # 2. PARAMETERS (Lyapunov Gains cho robot thật thường thấp hơn)
        ...
        self.declare_parameter('Kx', 2.0)   # Giảm nhẹ để mượt hơn
        self.declare_parameter('Ky', 40.0)  # Giảm từ 80 xuống 40 để tránh rung lắc
        ...
        self.last_log_t = -1
        self.data_log = []
        self.csv_filename = f'real_motion_law_lyapunov_log_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        ...
        self.get_logger().info(f"Lyapunov Node Ready.")
    ...
    def control_loop(self):
        ...
        ddx_ref, ddy_ref = d2x_ds2 * (vs**2) + dx_ds * as_val, d2y_ds2 * (vs**2) + dy_ds * as_val
        ...
        # C. ERROR TRANSFORMATION (Body Frame)
        ...
        c_q, s_q = math.cos(self.curr_q[2]), math.sin(self.curr_q[2])
        e1 =  c_q * ex_g + s_q * ey_g
        e2 = -s_q * ex_g + c_q * ey_g
        e3 = ephi_g
        ...
        # E. RATE LIMIT (Siết chặt hơn cho robot thật)
    ...
    def stop_robot(self):
        ...
        self.get_logger().info("=== TARGET REACHED / STOPPED ===")
    ...
    def update_path_and_log(self, t, e_dist, v, w):
        ...
        if int(t) > self.last_log_t:
            self.get_logger().info(f"Time: {t:.1f}s | Err: {e_dist:.3f}m | RMS: {rms_err:.3f}m | v: {v:.3f}m/s | w: {w:.3f}rad/s")
...
def main():
    rclpy.init()
    node = RealRobotLyapunovFollower()
    try: rclpy.spin(node)
