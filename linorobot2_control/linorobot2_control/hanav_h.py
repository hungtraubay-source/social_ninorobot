#!/usr/bin/env python3
"""
ha_nav2_expert_follower_v2_10terms.py
Bộ điều khiển bám quỹ đạo HA 10 Terms - Tối ưu hóa từ 12 terms
- Giảm 2 terms (VVSmall, VLSmall) → Phản ứng mượt hơn
- Hiển thị cross-track error và along-track error
- Log text thuần, tối ưu cho Terminator
"""
...
# ===========================================================================
#  HEDGE ALGEBRA (10 ngữ nghĩa) - TỐI ƯU HÓA
# ===========================================================================
class HedgeAlgebra:
    def __init__(self, theta=0.5, alpha=0.7, beta=0.3):
        ...
        # 10 ngữ nghĩa: BỎ VVSmall, VLSmall, VVLarge → Mượt hơn
        self.term_specs = [
            ('Zero',     [],       None   ),    # 0.0
            ('VSmall',   ['V'],    'Small'),    # ~0.10
            ('Small',    [],       'Small'),    # ~0.30
    ...
    def _sgn_base(self, h, base): 
        return -1 if h == 'V' else 1 if base == 'Small' else 1 if h == 'V' else -1
...
# ===========================================================================
#  RULE BASES (9 quy tắc cho 10 terms)
# ===========================================================================
def _rule_longitudinal():
    """Gia tốc/Giảm tốc dọc (e1)"""
    return {
        'Zero':     'Zero',
        'VSmall':   'Zero',
        'Small':    'VSmall',      # → VSmall (tối ưu từ Small của 12 terms)
        'LSmall':   'Small',
        'Neutral':  'LSmall',
        'LLarge':   'Large',
        'Large':    'VLarge',      # → VLarge
...
def _rule_lateral():
    ...
    return {
        ...
        'Small':    'VSmall',
        'LSmall':   'Small',
        'Neutral':  'Large',       # Tăng cứng hơn từ Neutral
        'LLarge':   'VLarge',
        'Large':    'VLarge',
        'VLarge':   'Absolute',
        'Absolute': 'Absolute'
...
def _rule_heading():
    ...
    return {
        ...
        'Small':    'Small',
        'LSmall':   'LSmall',
        'Neutral':  'Large',       # Hướng dẫn từ trung tâm
        'LLarge':   'LLarge',
        'Large':    'VLarge',
        'VLarge':   'Absolute',
        'Absolute': 'Absolute'
...
# ===========================================================================
#  HA CONTROLLER - 10 Terms
# ===========================================================================
class HAController:
    MAX_E1 = 10.0   # Sai số dọc (m)
    MAX_E2 = 10.0   # Sai số ngang (m)
    MAX_E3 = math.pi / 3  # Sai số hướng (rad)

    def __init__(self, ha, gain_e1=0.3, gain_e2=0.5, gain_e3=0.5,
                 v_max=0.15, w_max=2.0, dv_max=0.03, dw_max=4.0, Ts=0.05, lpf_alpha=0.85):
        self.ha         = ha
        self.gain_e1    = gain_e1       # Có thể tăng từ 0.3 → 0.4 do rules ít hơn
        self.gain_e2    = gain_e2       # Có thể tăng từ 1.5 → 1.8
        self.gain_e3    = gain_e3       # Giữ nguyên hoặc tăng 0.5 → 0.6
        self.v_max      = v_max
        self.w_max      = w_max
        self.dv_max     = dv_max
        self.dw_max     = dw_max
...
class HANav2ExpertNode(Node):
    ...
    def __init__(self):
        ...
        self.look_ahead_dist = 0.25
        self.curve_beta      = 0.15
        self.num_curve_samp  = 8

        self.get_logger().info(f'HA 10 Terms (OPTIMIZED) | Ts={self.Ts}s | v_max={self.v_path_max}m/s')

        self.traj = None
        self.ha   = HedgeAlgebra()
        self.ctrl = HAController(
            ha=self.ha,
            gain_e1=0.3,   # Tăng từ 0.3 (rules ít hơn)
            gain_e2=1.2,   # Tăng từ 1.5
            gain_e3=0.15,   # Tăng từ 0.5
            v_max=0.15,    w_max=2.0,
            dv_max=2.00,  dw_max=10.00,
            Ts=self.Ts,   lpf_alpha=0.85)
        ...
        self.get_logger().info('Node HA Nav2 Follower (10 Terms) đã sẵn sàng.')
    ...
    def path_callback(self, msg: Path):
        ...
        if self.pose_received:
            ...
            self.get_logger().info(f'Khởi tạo s = {self.s:.2f} m (pose đã có)')
        else:
            self.s = 0.0
            self.s_initialized = False
            self.get_logger().info('Path nhận trước pose, sẽ khởi tạo s khi có pose.')
    ...
    def control_loop(self):
        ...
        if not self.s_initialized:
            ...
            self.s_initialized = True
            self.get_logger().info(f'Khởi tạo s = {self.s:.2f} m (pose vừa nhận)')

        # Cập nhật virtual target s và v_s
