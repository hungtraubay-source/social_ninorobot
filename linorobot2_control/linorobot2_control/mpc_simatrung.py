#!/usr/bin/env python3
"""
mpc_standalone_follower_synced.py
Bộ điều khiển MPC chạy Standalone (Mô phỏng quỹ đạo số 8)
Đã đồng bộ thông số thực chiến từ bản Nav2 v3
"""
...
class MPCFollower(Node):
    def __init__(self):
        ...
        # =====================================================================
        # [ĐỒNG BỘ NAV2 v3] - THÔNG SỐ VẬT LÝ & ĐỘNG HỌC THỰC CHIẾN
        # =====================================================================
        # [FIX 2] AGGRESSIVE SMOOTHING: ar từ 0.65 → 0.15
        self.ar = 0.15
        
        # [FIX 2] Giới hạn vận tốc & gia tốc - Đồng bộ từ Nav2 Expert v3
        self.v_max = 0.25            # Tốc độ thẳng max (m/s)
        self.w_max = 1.5             # Tốc độ góc max (rad/s)
        self.dv_max = 0.8            # Gia tốc thẳng max (m/s²)
        self.dw_max = 4.0            # Gia tốc góc max (rad/s²)
        
        # [FIX 2] Vận tốc mục tiêu ảo
        self.v_path_max = 0.25       # Vận tốc mục tiêu ảo (m/s)
        self.a_path_max = 0.20       # Gia tốc của mục tiêu ảo (m/s²)

        # [ĐỒNG BỘ NAV2 v3] - TRỌNG SỐ BRYSON'S RULE (HA Synced)
        self.e_xy_max = 0.1          # Dung sai lệch ngang/dọc (10cm)
        self.e_phi_max = math.pi / 4 # Dung sai lệch góc (45 độ = π/4 rad)
        self.vr_max = self.v_max / 2.0     # v_max/2
        self.wr_max = self.w_max / 2.0     # w_max/2
        # =====================================================================
        ...
        self.last_log_t = -1
        self.data_log = []
        self.csv_filename = f'motion_law_mpc_log_synced_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        ...
        self.get_logger().info(
            f"MPC Standalone (HA Synced) Ready. v_max={self.v_max}m/s | ar={self.ar} | Play Gazebo!"
        )
    ...
    def precompute_path(self):
        ...
        while s_tmp <= self.s_end:
            ...
            elif dist < 0.5: a_s = -a_path_max
            elif v_s > v_limit + 0.01: a_s = -a_path_max 
            elif v_s < v_limit - 0.01: a_s = a_path_max  
            else: a_s = 0.0
