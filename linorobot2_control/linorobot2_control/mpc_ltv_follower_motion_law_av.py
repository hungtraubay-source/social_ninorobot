...
class MPCFollower(Node):
    def __init__(self):
        ...
        self.last_log_t = -1
        self.data_log = []
        self.csv_filename = f'motion_law_mpc_log_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        ...
        self.get_logger().info(f"MPC Node Ready. Play Gazebo!")
    ...
    def get_lookahead_safe_velocity(self, s_curr):
        ...
        beta = 0.01              # Độ nhạy (tăng lên nếu muốn cua chậm hơn)
    ...
    def control_loop(self):
        ...
        r_ramp = 5.0 - 4.0 * min(t / 3.0, 1.0) # Giảm từ 5.0 xuống 1.0 trong 3 giây
